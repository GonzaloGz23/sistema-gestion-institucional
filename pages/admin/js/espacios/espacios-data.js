const cargarEdificiosDisponibles = async (idSeleccionado = null) => {
    ...
    const btnGuardarEspacio = document.getElementById("modalEspacioConfirm");
    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            ...
            if (edificiosHabilitados.length > 0) {
                ...
                btnGuardarEspacio.disabled = false; // Habilitar el botón de guardar 
                ...
                btnGuardarEspacio.disabled = true; // Deshabilitar el botón de guardar
                if (mensajeSinEdificios) {
...
const cargarEspacios = async () => {
    const tablaBody = document.getElementById("tablaEspaciosBody");
    ...
    try {
        ...
        // 🔥 Si ya existe un DataTable anterior, destruirlo
        ...
        if (result.success && result.data.length > 0) {
            result.data.forEach((espacio, index) => {
                const fila = `
                    <tr>
                        ...
                        <td>${espacio.alias}</td>
                        <td>${espacio.detalle || "Sin detalles"}</td>
                        <td>${espacio.edificio || "Sin edificio"}</td>
                        <td>${espacio.estado === "habilitado" ? " <span class='badge bg-primary-soft'>Habilitado</span>" : "<span class='badge bg-secondary-soft'>Deshabilitado</span>"}</td>
                        <td>
                            <button class="btn btn-primary btn-sm m-1" title="Editar" 
                                data-bs-toggle="modal" 
                                data-bs-target="#modalEspacio"
                                data-action="editar" 
                                data-id="${espacio.id_espacio}" 
                                data-nombre="${espacio.alias}"
                                data-detalles="${espacio.detalle || ''}" 
                                data-edificio="${espacio.id_edificio || ''}"
                            ...
                            <button 
                               class="btn ${espacio.estado === "habilitado" ? "btn-warning" : "btn-success"} btn-sm m-1" 
                                title="${espacio.estado === "habilitado" ? "Deshabilitar" : "Habilitar"}" 
                                data-bs-toggle="modal" 
                                data-bs-target="#modalEspacio"
            ...
            mensajeNoEspacios.classList.add("d-none");
            ...
            mensajeNoEspacios.classList.remove("d-none");
...
const agregarEspacio = async () => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/espacios/agregar_espacios.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Espacio agregado correctamente", "¡Éxito!");

            document.getElementById("modalEspacio").querySelector(".btn-close").click();
            cargarEspacios();
        } else {
        ...
        mostrarAlerta("error", "Hubo un problema al agregar el espacio", "¡Error!");
    }
};
...
const editarEspacio = async () => {
    const form = document.getElementById("formEspacio");
    const formData = new FormData(form);
    const idEspacio = form.getAttribute("data-id");

    formData.append("id", idEspacio);

    try {
        const response = await fetch("../../backend/controller/admin/espacios/editar_espacios.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Espacio actualizado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al editar el espacio", "¡Error!");
    }
};

const habilitarDeshabilitarEspacio = async (id, estadoActual) => {
    const formData = new FormData();
    formData.append("id", id);
    formData.append("estado", estadoActual);

    try {
        const response = await fetch("../../backend/controller/admin/espacios/habilitar_deshabilitar_espacios.php", {
            method: "POST",
            body: formData
        });
        ...
        mostrarAlerta("error", "Hubo un problema al actualizar el espacio", "¡Error!");
    }
};
...
const eliminarEspacio = async (id) => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/espacios/eliminar_espacios.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Espacio eliminado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al eliminar el espacio", "¡Error!");
    }
};
