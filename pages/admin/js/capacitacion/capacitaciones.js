document.addEventListener('DOMContentLoaded', function() {
    ...
    const radioAsincrona = document.getElementById('radioDefault3');
    ...
    function toggleLugarVisibility() {
        ...
        } else if (radioAsincrona.checked) {
            lugarDiv.style.display = 'none';
            enlaceDiv.style.display = 'none'; 
        } else {
            lugarDiv.style.display = 'none';
    ...
    radioAsincrona.addEventListener('change', toggleLugarVisibility);
...
form.addEventListener("submit", async (event) => {
    ...
    const selectColaborador = document.querySelector('.js-example-basic-single'); // Obtener el elemento select
     ...
     const GEquipos = document.getElementById('tGrupos');
    ...
    selectedItemsElements.forEach(itemElement => {
        ...
        const selectGrupo = document.getElementById('selectGrupos');
    const checkboxEquipo = document.getElementById('Equipos'); // Obtenemos el checkbox

            const nuevoGrupo = document.getElementById('nuevoS');
   const GNuevo = document.getElementById('nuevoS'); // Obtenemos el checkbox
    const GNuevo2 = document.getElementById('nuevoB');

    // Verificar si no se ha seleccionado ningún colaborador, equipo o "Aplicar a todos"
   
if (checkboxEquipo.checked && selectGrupo.value === "") {
            Swal.fire({
                title: "Advertencia",
                text: "Si ha marcado 'Mostrar Grupos', debe seleccionar un grupo.",
                icon: "warning"
            });
            return; // Detener el envío del formulario
        } else if(colaboradoresIds.length === 0 && equiposIds.length === 0 && !aplicarATodos) {

 // Obtiene los estilos computados del elemento
    const estiloComputado = window.getComputedStyle(nuevoGrupo);

    // Accede a la propiedad 'display'
    const displayStyle = estiloComputado.display;

    // Comprueba si 'display' no es 'none'
    ...
    formData.append("lugar", document.getElementById("lugar").value);
        ...
        formData.append("grupo", document.getElementById('selectGrupos').value);
    ...
    if (result.success) {
        Swal.fire({
            ...
            GEquipos.style.display = 'none';
            GNuevo.style.display = 'block';
            GNuevo2.style.display = 'block';
...
async function loadcapacitaciones() {
    ...
    try {
        ...
        const response = await fetch(`../../backend/controller/usuario/capacitacion/conseguir_capacitacion.php?idUserx=${encodeURIComponent(idUserx)}&t=${timestamp}`, {
            headers: {
        ...
        console.log('📋 Capacitaciones cargadas:', capacitacion.length);
...
async function openAddCapacitacionModal(capacitacionId) {
 ...
 <div class="form-check">
        <input class="form-check-input" type="radio" name="modalidad" id="asincrona-${noteCapacitacionId}" value="Asincrona" ${note.modalidad === 'Asincrona' ? 'checked' : ''}>
        <label class="form-check-label" for="asincrona-${noteCapacitacionId}">
            Asincrona
        </label>
    </div>
...
<div class="mb-2"> 
                                            <label class="form-check-label" for="obligacion-${noteCapacitacionId}"><b>Obligacion</b></label>

   <select id="obligacion-${noteCapacitacionId}" name="newNoteTag" class="form-control">
   ${note.obligacion === 'obligatorio' ? ' <option value="obligatorio" selected>obligatorio</option> <option value="optativo">optativo</option>' : '<option value="obligatorio">obligatorio</option> <option value="optativo" selected>optativo</option>'}
...
<select multiple name="drawfs" id="drawfs" class="form-control">
  ...
  // --- *** APLICAR LÓGICA DE VALIDACIÓN DE FECHAS A LOS INPUTS DEL MODAL *** ---
        ...
        const obligacionModal = newModalElement.querySelector(`#obligacion-${noteCapacitacionId}`);
...
console.log('ID Capacitación del material:', capacitacionIdToDelete);
  ...
  spinnerActualizar.classList.remove('d-none'); // Muestra el spinner
                 ...
                 const oblig = obligacionModal.value;
                ...
                formData.append('obligacion', oblig);
...
document.addEventListener('DOMContentLoaded', function() {
    const selectGrupos = document.getElementById('selectGrupos');
    const checkboxEquipos = document.getElementById('Equipos'); // Obtenemos el checkbox
    const GEquipos = document.getElementById('tGrupos'); // Obtenemos el checkbox

    const GNuevo = document.getElementById('nuevoS'); // Obtenemos el checkbox
    const GNuevo2 = document.getElementById('nuevoB'); // Obtenemos el checkbox


    // --- Función para cargar los grupos (la misma que antes) ---
    async function cargarGrupos() {
        try {
            const response = await fetch('../../backend/controller/usuario/capacitacion/get_grupos.php');
            ...
            // Si ya tienes la opción "Seleccionar Grupos" en el HTML, puedes omitir esta línea
            // o ajustarla para que no la elimine.
            // while (selectGrupos.children.length > 1) {
            //     selectGrupos.removeChild(selectGrupos.lastChild);
            // }
            ...
            alert('No se pudieron cargar los grupos. Por favor, inténtalo de nuevo más tarde.');
        }
    }

    // --- Lógica para el checkbox y el select ---

    // Añadir un "event listener" al checkbox
    checkboxEquipos.addEventListener('change', function() {
        if (checkboxEquipos.checked) {
            // Si el checkbox está marcado, mostramos el select
            selectGrupos.style.display = 'block';
            GEquipos.style.display = 'block';
            GNuevo.style.display = 'none';
            GNuevo2.style.display = 'none';
            // Opcional: Si quieres cargar los grupos solo cuando se muestra, llama aquí cargarGrupos()
            // cargarGrupos();
        } else {
            // Si el checkbox no está marcado, ocultamos el select
            selectGrupos.style.display = 'none';
            GEquipos.style.display = 'none';
             GNuevo.style.display = 'block';
            GNuevo2.style.display = 'block';
        }
    ...
    // Opcional: Puedes llamar a cargarGrupos() al inicio si quieres que estén listos
    // aunque el select esté oculto, o solo cuando se muestre por primera vez.
    // Si la lista de grupos no cambia a menudo, cargarla al inicio es eficiente.
    // Si cambia mucho, podrías cargarla solo cuando el checkbox se marque por primera vez.
    cargarGrupos(); // Cargamos los grupos al inicio, aunque el select esté oculto.
});