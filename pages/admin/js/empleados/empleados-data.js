const cargarRolesDisponibles = async (idRolSeleccionado = null) => {
    const selectRol = document.getElementById("selectRol");
    const mensajeSinRoles = document.getElementById("mensajeSinRoles");
    const btnGuardar = document.getElementById("modalEmpleadoConfirm");

    try {
        const response = await fetch("../../backend/controller/admin/roles/listar_roles.php");
        ...
        if (result.success && result.roles.length > 0) {
            result.roles.forEach((rol) => {
                ...
                option.textContent = rol.alias;
                if (idRolSeleccionado && rol.id_rol === parseInt(idRolSeleccionado)) {
            ...
            mensajeSinRoles.classList.add("d-none");
            btnGuardar.disabled = false;
        } else {
            selectRol.classList.add("d-none");
            mensajeSinRoles.classList.remove("d-none");
...
const cargarEquiposDisponibles = async (idSeleccionado = null) => {
    const selectEquipos = document.getElementById("equipoEmpleado");
    const mensajeSinEquipos = document.getElementById("mensajeSinEquipos");
    const btnGuardarEmpleado = document.getElementById("modalEmpleadoConfirm");

    try {
        const response = await fetch("../../backend/controller/admin/equipos/listar_equipos.php");
        const result = await response.json();

        selectEquipos.innerHTML = ""; // Limpiar el select antes de agregar opciones

        if (result.success && result.data.length > 0) {
            const equiposHabilitados = result.data.filter(equipo => equipo.estado === "habilitado");

            if (equiposHabilitados.length > 0) {
                equiposHabilitados.forEach((equipo) => {
                    const option = document.createElement("option");
                    option.value = equipo.id_equipo;
                    option.textContent = equipo.alias;
                    if (idSeleccionado && equipo.id_equipo == idSeleccionado) {
                        option.selected = true;
                    }
                ...
                btnGuardarEmpleado.disabled = false; // Habilitar el botón de guardar 

                if (mensajeSinEquipos) {
                    mensajeSinEquipos.classList.add("d-none"); // Ocultar mensaje de error
                }
            } else {
                selectEquipos.classList.add("d-none");
                btnGuardarEmpleado.disabled = true; // Deshabilitar el botón de guardar
                if (mensajeSinEquipos) {
                    mensajeSinEquipos.classList.remove("d-none"); // Mostrar mensaje de error
                }
            }
        } else {
            selectEquipos.classList.add("d-none");
...
const cargarEdificiosDisponibles = async (idSeleccionado = null) => {
    const selectEdificios = document.getElementById("edificioEmpleado");
    const mensajeSinEdificios = document.getElementById("mensajeSinEdificios");

    try {
        const response = await fetch("../../backend/controller/admin/edificios/listar_edificios.php");
        const result = await response.json();

        selectEdificios.innerHTML = ""; // Limpiar el select antes de agregar opciones

        if (result.success && result.data.length > 0) {
            const edificiosHabilitados = result.data.filter(edificio => edificio.estado === "habilitado");

            if (edificiosHabilitados.length > 0) {
                edificiosHabilitados.forEach((edificio) => {
                    const option = document.createElement("option");
                    option.value = edificio.id_edificio;
                    option.textContent = edificio.alias;
                    if (idSeleccionado && edificio.id_edificio == idSeleccionado) {
                        option.selected = true;
                    }
                    selectEdificios.appendChild(option);
                });

                selectEdificios.classList.remove("d-none");

                if (mensajeSinEdificios) {
                    mensajeSinEdificios.classList.add("d-none"); // Ocultar mensaje de error
                }
            } else {
                selectEdificios.classList.add("d-none");
                if (mensajeSinEdificios) {
                    mensajeSinEdificios.classList.remove("d-none"); // Mostrar mensaje de error
                }
            }
        } else {
            selectEdificios.classList.add("d-none");
            if (mensajeSinEdificios) {
                mensajeSinEdificios.classList.remove("d-none");
            }
        }
    } catch (error) {
...
const cargarEmpleados = async () => {
    ...
    const mensajeNoEmpleados = document.getElementById("mensajeNoEmpleados");
    ...
    try {
        ...
        const response = await fetch("../../backend/controller/admin/empleados/listar_empleados.php");
        const result = await response.json();
        ...
        if (result.success && result.data.length > 0) {
            result.data.forEach((empleado, index) => {
                const fila = `
                    <tr>
                        <th scope="row">${index + 1}</th>
                        <td>${empleado.nombre} ${empleado.apellido}</td>
                        <td>${empleado.dni || "Sin DNI"}</td>
                        <td>${empleado.equipo || "Sin equipo"}</td>
                        <td>${empleado.edificio || "Sin edificio"}</td>
                        <td>${empleado.estado === "habilitado" ? " <span class='badge bg-primary-soft'>Habilitado</span>" : "<span class='badge bg-secondary-soft'>Deshabilitado</span>"}</td>
                        <td>
                            <button class="btn btn-primary btn-sm m-1" title="Editar" 
                                ...
                                data-id-rol="${empleado.id_rol}"
                                data-id-equipo="${empleado.id_equipo || ''}"
                                data-id-edificio="${empleado.id_edificio || ''}">
                            ...
                            <button 
                               class="btn ${empleado.estado === "habilitado" ? "btn-warning" : "btn-success"} btn-sm m-1" 
                                title="${empleado.estado === "habilitado" ? "Deshabilitar" : "Habilitar"}" 
                                data-bs-toggle="modal" 
                                data-bs-target="#modalEmpleado"
            ...
            mensajeNoEmpleados.classList.add("d-none");
            ...
            mensajeNoEmpleados.classList.remove("d-none");
...
const agregarEmpleado = async () => {
    ...
    const mainElement = document.querySelector('main.db-content');
    const valorEntidad = mainElement.dataset.entity;

    // Se asigna temporalmente el ID de rol 4 hasta que se implemente el módulo de roles
    formData.append("id_entidad", valorEntidad);

    try {
        const response = await fetch("../../backend/controller/admin/empleados/agregar_empleados.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Empleado agregado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al agregar el empleado", "¡Error!");
    }
};
...
const editarEmpleado = async (id) => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/empleados/editar_empleados.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Empleado actualizado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al editar el empleado", "¡Error!");
    }
};

const habilitarDeshabilitarEmpleado = async (id, estadoActual) => {
    const formData = new FormData();
    formData.append("id", id);
    formData.append("estado", estadoActual);

    try {
        const response = await fetch("../../backend/controller/admin/empleados/habilitar_deshabilitar_empleados.php", {
            method: "POST",
            body: formData
        });
        ...
        mostrarAlerta("error", "Hubo un problema al actualizar el empleado", "¡Error!");
    }
};
...
const eliminarEmpleado = async (id) => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/empleados/eliminar_empleados.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Empleado eliminado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al eliminar el empleado", "¡Error!");
    }
};
