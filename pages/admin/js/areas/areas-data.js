const cargarAreas = async () => {
    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            result.data.forEach((area, index) => {
                const fila = `
                    <tr>
                        <th scope="row">${index + 1}</th>
                        <td>${area.alias}</td>
                        <td>${area.entidad || "Sin entidad"}</td>
                        <td>${area.estado === "habilitado" ? " <span class='badge bg-primary-soft'>Habilitado</span>" : "<span class='badge bg-secondary-soft'>Deshabilitado</span>"}</td>
                        <td>
                            <button class="btn btn-primary btn-sm m-1" title="Editar" 
                                ...
                                data-nombre="${area.alias}"
                                data-entidad="${area.entidad || ''}"
                            ...
                            <button 
                               class="btn ${area.estado === "habilitado" ? "btn-warning" : "btn-success"} btn-sm m-1" 
                                title="${area.estado === "habilitado" ? "Deshabilitar" : "Habilitar"}" 
                                data-bs-toggle="modal" 
                                data-bs-target="#modalArea"
            ...
            mensajeNoAreas.classList.add("d-none");

            // 🔥 Inicializar DataTablesJS
            $(tabla).DataTable({
                pageLength: 10,
...
const agregarArea = async () => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/areas/agregar_areas.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Área agregada correctamente", "¡Éxito!");

            document.getElementById("modalArea").querySelector(".btn-close").click();
            cargarAreas();
        } else {
        ...
        mostrarAlerta("error", "Hubo un problema al agregar el área", "¡Error!");
    }
};
...
const editarArea = async () => {
    ...
    const mainElement = document.querySelector('main.db-content');
    const valorEntidad = mainElement.dataset.entity;
    const idArea = form.getAttribute("data-id");

    formData.append('id_entidad', valorEntidad);
    formData.append("id", idArea);

    try {
        const response = await fetch("../../backend/controller/admin/areas/editar_areas.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Área actualizada correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al editar el área", "¡Error!");
    }
};

const habilitarDeshabilitarArea = async (id, estadoActual) => {
    const formData = new FormData();
    formData.append("id", id);
    formData.append("estado", estadoActual);

    try {
        const response = await fetch("../../backend/controller/admin/areas/habilitar_deshabilitar_areas.php", {
            method: "POST",
            body: formData
        });
        ...
        mostrarAlerta("error", "Hubo un problema al actualizar el área", "¡Error!");
    }
};
...
const eliminarArea = async (id) => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/areas/eliminar_areas.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Área eliminada correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al eliminar el área", "¡Error!");
    }
};
