const cargarEdificios = async () => {
    const tablaBody = document.getElementById("tablaEdificiosBody");
    const contenedorTabla = document.getElementById("contenedorTabla");
    const mensajeNoEdificios = document.getElementById("mensajeNoEdificios");
    ...
    try {
        ...
        if (result.success && result.data.length > 0) {
            result.data.forEach((edificio, index) => {
                const fila = `
                    <tr>
                        <th scope="row">${index + 1}</th>
                        <td>${edificio.alias}</td>
                        <td>${edificio.direccion || "Sin dirección"}</td>
                        <td>${edificio.entidad || "Sin entidad"}</td>
                         <td>${edificio.estado === "habilitado" ? " <span class='badge bg-primary-soft'>Habilitado</span>" : "<span class='badge bg-secondary-soft'>Deshabilitado</span>"}</td>
                        <td>
                            <button class="btn btn-primary btn-sm m-1" title="Editar" 
                                ...
                                data-nombre="${edificio.alias}"
                                data-direccion="${edificio.direccion || ''}" 
                                data-entidad="${edificio.entidad || ''}"
                                data-estado="${edificio.estado}">
                            ...
                            <button 
                               class="btn ${edificio.estado === "habilitado" ? "btn-warning" : "btn-success"} btn-sm m-1" 
                                title="${edificio.estado === "habilitado" ? "Deshabilitar" : "Habilitar"}" 
                                data-bs-toggle="modal" 
                                data-bs-target="#modalEdificio"
            ...
            mensajeNoEdificios.classList.add("d-none");
            ...
            mensajeNoEdificios.classList.remove("d-none");
...
const agregarEdificio = async () => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/edificios/agregar_edificios.php", { // Nueva ruta
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Edificio agregado correctamente", "¡Éxito!");

            // **Cerrar el modal**
            document.getElementById("modalEdificio").querySelector(".btn-close").click();
        ...
        mostrarAlerta("error", "Hubo un problema al agregar el edificio", "¡Error!");
    }
};

const editarEdificio = async () => {
    const form = document.getElementById("formEdificio");
    const formData = new FormData(form);
    const mainElement = document.querySelector('main.db-content');
    const valorEntidad = mainElement.dataset.entity;
    const idEdificio = form.getAttribute("data-id");

    formData.append('id_entidad', valorEntidad);
    formData.append("id", idEdificio);

    try {
        const response = await fetch("../../backend/controller/admin/edificios/editar_edificios.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Edificio actualizado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al editar el edificio", "¡Error!");
    }
};

const habilitarDeshabilitarEdificio = async (id, estadoActual) => {
    const formData = new FormData();
    formData.append("id", id);
    formData.append("estado", estadoActual);

    try {
        const response = await fetch("../../backend/controller/admin/edificios/habilitar_deshabilitar_edificios.php", {
            method: "POST",
            body: formData
        });
        ...
        mostrarAlerta("error", "Hubo un problema al actualizar el edificio", "¡Error!");
    }
};
...
const eliminarEdificio = async (id) => {
    ...
    try {
        const response = await fetch("../../backend/controller/admin/edificios/eliminar_edificios.php", {
            method: "POST",
            body: formData
        });
        ...
        if (result.success) {
            mostrarAlerta("success", "Edificio eliminado correctamente", "¡Éxito!");
        ...
        mostrarAlerta("error", "Hubo un problema al eliminar el edificio", "¡Error!");
    }
};
