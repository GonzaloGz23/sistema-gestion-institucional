...
if (!empty($_POST["id"]) && !empty($_POST["estado"])) {
    ...
    try {
        $stmt = $pdo->prepare(
            "UPDATE `formularios` SET `estado` = :estado WHERE `id_formularios` = :id",
        );
            ...
            if (count($datos_form) > 0) {
                foreach ($datos_form as $s) {
                    switch ($s["id_tipos_formularios"]) {
                        case 1:
                            $url =
                                "http://localhost/sistema-gestion-institucional/pages/user/form_legajos.php";
                            break;

                        case 2:
                            $url =
                                "http://localhost/sistema-gestion-institucional/pages/user/rrhh.php";
                            break;

                        case 3:
                            $url =
                                "http://localhost/sistema-gestion-institucional/pages/user/form_prestaciones.php";
                            break;

                        case 4:
                            $url =
                                "http://localhost/sistema-gestion-institucional/pages/user/form_resultados.php";
                            break;
                    ...
                    $icon =
                        "https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.5/icons/ui-checks.svg";
